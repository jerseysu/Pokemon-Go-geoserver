...
static void fireClick(CFRunLoopTimerRef timer __unused, void *info)
{
   if (!isPaused)
   {
        postMouseEvent(kCGMouseButtonLeft, kCGEventLeftMouseDown, firstPoint);
        usleep(100 * 1000);
        postMouseEvent(kCGMouseButtonLeft, kCGEventLeftMouseUp, firstPoint);
        usleep(100 * 1000);
        postMouseEvent(kCGMouseButtonLeft, kCGEventLeftMouseDown, secondPoint);
        usleep(100 * 1000);
        postMouseEvent(kCGMouseButtonLeft, kCGEventLeftMouseUp, secondPoint);
   }
}
...
int main(int argc, char *argv[]) {
    @autoreleasepool {
        ...
        isPaused = YES;
        x1 = [args integerForKey:@"x1"];
        y1 = [args integerForKey:@"y1"];
        x2 = [args integerForKey:@"x2"];
        y2 = [args integerForKey:@"y2"];
        pt.x = x1;
        pt.y = y1;
        firstPoint = pt;
...
CGEventRef myCGEventCallback (CGEventTapProxy proxy, CGEventType type, CGEventRef event, void *refcon) {
    ...
    printf("[code:%d] %s\n", keyCode,keyCodeToReadableString(keyCode));
    if (keyCode == 56)
    {
        /* start */
        isPaused = NO;
